...
function append_api_sfct_authed(app, db, config){
	...
	} else if(!req.auth.editor){
	    res.status(500).end('this user can not edit trans.')
	} else {
	    let token = req.token
	...
	} else if(!req.auth.voter){
	    res.status(500).end('this user can not vote.')
	} else {
	    let trans_id = ObjectID(req.args.trans_id)
...
module.exports = append_api_sfct_authed
